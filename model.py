...
class GPT2AttentionXWWX(nn.Module):
    ...
    def _xwwx_attn(self, hidden_states, encoder_hidden_states, w_q, w_k, w_v, q_bias, k_bias, v_bias, attention_mask=None, head_mask=None):
        ...
        hidden_states = hidden_states.view(bsz, -1, hidden_states.size(-1))

        # matmul
        # step2 计算x*w_k^T
        # x:[bsz, seq_len, embed_dim],w_k:[embed_dim, embed_dim], k_bias:[embed_dim,]
        # ??? k_bias? k_bias^T?
        k_bias_T = torch.matmul(hidden_states, k_bias.unsqueeze(
            0).transpose(0, 1))  # [bsz,q_seq_len,1]
        hidden_states = torch.matmul(hidden_states, w_k.transpose(0, 1))

        # #addmm
        # hidden_states = torch.addmm(k_bias.T, hidden_states.view(-1, hidden_states.size(-1)), w_k.transpose(0, 1))
        # hidden_states = hidden_states.view(bsz, -1, hidden_states.size(-1))
        ...
        with torch.amp.autocast(hidden_states.device.type, enabled=False):
            ...
            # 使用 matmul 替代 baddbmm

            k_bias_T = k_bias_T.unsqueeze(1)
            k_bias_T = k_bias_T.expand(-1, self.num_heads, -1, -1)
            k_bias_T = k_bias_T.reshape(-1, q_seq_len, 1)
            attn_weights = torch.matmul(x, x_t) + k_bias_T  # 核心修改点
    ...
    def _xwwx_attn_bias(self, hidden_states, encoder_hidden_states, encoder_attention_mask, attention_mask=None, head_mask=None):
        ...
        """(x*W_q + q_bias)(x*W_k + k_bias)^T === (x*W_q + q_bias)(W_k^T*x^T + k_bias^T) ===  (x*W_q + q_bias)W_k^T*x^T +  (x*W_q + q_bias)*k_bias^T"""
        ...
        tmp1 = torch.addmm(
            q_bias, hidden_states.view(-1, hidden_states.size(-1)), w_q)
        tmp1 = tmp1.view(bsz, -1, hidden_states.size(-1))

        with torch.amp.autocast(hidden_states.device.type, enabled=False):
            # matmul
            # step2 计算tmp1*w_k^T 和tmp1*k_bias^T
            # x:[bsz, q_seq_len, embed_dim],w_k:[embed_dim, embed_dim], k_bias:[embed_dim,]
            k_bias_T = torch.matmul(tmp1, k_bias.unsqueeze(
                0).transpose(0, 1))  # [bsz,q_seq_len,1]
            # [bsz,q_seq_len,embed_dim]
            tmp2 = torch.matmul(tmp1, w_k.transpose(0, 1))

            # [bsz, embed_dim, k_seq_len]
            x_t = encoder_hidden_states.transpose(-1, -2)
            # [bsz,q_seq_len,k_seq_len]
            attn_weights = torch.matmul(tmp2, x_t) + k_bias_T
        ...
        # [bsz,num_heads,q_seq_len,k_seq_len]
        attn_weights = attn_weights.expand(-1, self.num_heads, -1, -1)
    ...
    def _ori_attn(self, hidden_states, encoder_hidden_states, encoder_attention_mask, attention_mask=None, head_mask=None):
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    "If class is used as cross attention, the weights `q_attn` have to be defined. "
    ...
    def forward(
        ...
        **kwargs,
        ...
        #     w_q = self.q_attn.weight
        #     q_bias = self.q_attn.bias
        #     # nn.init.normal_(w_q, std=0.02)
        #     # 从c_attn中提取W_k和W_v
        #     # c_attn.weight的形状是[embed_dim, 2*embed_dim]
        #     w_k, w_v = self.c_attn.weight.split(self.split_size, dim=1)
        #     # c_attn.bias的形状是[2*embed_dim,]
        #     k_bias, v_bias = self.c_attn.bias.split(self.split_size, dim=0)
        #     # nn.init.normal_(w_k, std=0.02)
        #     # nn.init.normal_(w_v, std=0.02)
        #     attention_mask = encoder_attention_mask
        # else:
        # # 自注意力机制：Q、K、V都来自同一个输入
