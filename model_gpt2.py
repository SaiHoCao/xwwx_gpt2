...
class GPT2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 稀疏x_kv,计算10%分位数作为阈值
        threshold = torch.quantile(inputx_kv.abs().flatten(), 0.1)
        ...
        query_states, _ , _ = self.c_attn(hidden_states).split(self.split_size, dim=2)
        _, key_states,value_states = self.c_attn(inputx_kv).split(self.split_size, dim=2)
